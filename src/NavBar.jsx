import React, { Component } from 'react';
import './NavBar.css';
import { Navbar, NavbarBrand, NavItem, NavbarToggler, Collapse } from 'reactstrap';
import { NavLink } from 'react-router-dom';
import { NavbarNav } from 'mdbreact';

class NavBar extends Component {
    constructor(props) {
        super(props);

        this.toggleNavbar = this.toggleNavbar.bind(this);
        this.state = {
            collapsed: true
        };
    }

    toggleNavbar() {
        this.setState({
            collapsed: !this.state.collapsed
        });
    }
    render() {
        return (
            <Navbar className="navcolor" expand="xs" scrolling fixed='top'>
                <NavbarBrand href="/" >
                    <img className='logo' src='/medias/LogoEpidemiumV2.png' alt='Logo' />
                </NavbarBrand>
                <NavbarToggler onClick={this.toggleNavbar} className="mr-2" />
                <Collapse isOpen={!this.state.collapsed} navbar>
                
                    <NavbarNav left  >
                        <NavItem className="links" ><NavLink style={{ color: 'white' }} to="/">
                            Créer son graphique
                        </NavLink>
                        </NavItem>
                        <NavItem className="links"><NavLink style={{ color: 'white' }} to="GraphPage">
                            Tous les graphiques
                        </NavLink>
                        </NavItem>
                        <NavItem className="links"><NavLink style={{ color: 'white' }} to="Society">
                            A propos
                        </NavLink>
                        </NavItem>

                    </NavbarNav>
                    <NavbarNav right>
                        <NavItem className="search-top">
                            <form className="form-inline md-form mt-0">
                                <input placeholder="Search..." />
                            </form>
                        </NavItem>
                        <p className="nav-p">Connecté</p>
                        <img className="img-spaced" src="data:image/png;base64," alt="changer langue" />
                    </NavbarNav>
                </Collapse>
            </Navbar>
        );
    }
}
export default NavBar;